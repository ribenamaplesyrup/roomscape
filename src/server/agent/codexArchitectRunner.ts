...
function buildArchitectPrompt(input: ArchitectRunInput, currentScene: string): string {
  return [
    ...
    "Default to the most immersive version of the requested change that remains performant: strong silhouettes, real-world proportions, layered construction, procedural texture detail, careful material roughness/metalness, and lighting-aware placement.",
...
function buildRepairPrompt(input: ArchitectRunInput, invalidScene: string, validationErrors: string[]): string {
  return [
    ...
    "Repairs should keep or improve the visual ambition of the requested change; do not downgrade to a crude placeholder merely to pass validation.",
    "Keep the module contract: export const roomTitle = string; export function buildRoom({ THREE, root, scene }: RoomSceneContext): void.",
