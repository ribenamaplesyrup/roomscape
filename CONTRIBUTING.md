...
3. Keep generated room code concerns inside `sandbox/rooms/active` and trusted app concerns inside `src/server` or `src/client`.
4. Run the focused tests, then `npm run test`.
5. Run `npm run build` for changes that touch shared contracts, server bootstrap, Vite config, or generated scene typing.
...
Roomscape's core safety property is that the live Codex runner can edit only the active room sandbox. Changes that affect this area need extra care:

- Preserve `sandbox/rooms/active` as the live working directory.
